...
public final class MyInstanceTree {
	private final MyModelTree source;
	@Nullable
	private final TransformedInstance instance;
	private final MyInstanceTree[] children;
	...
	private MyInstanceTree(MyModelTree source, @Nullable TransformedInstance instance, MyInstanceTree[] children) {
		this.source = source;
	...
	public static MyInstanceTree create(InstancerProvider provider, MyModelTree meshTree) {
		MyInstanceTree[] children = new MyInstanceTree[meshTree.childCount()];
		...
		return new MyInstanceTree(meshTree, instance, children);
	}
	...
	public MyInstanceTree child(int index) {
	...
	@Nullable
	public MyInstanceTree child(String name) {
	...
	public MyInstanceTree childOrThrow(String name) {
		MyInstanceTree child = child(name);
	...
	public void traverse(Consumer<? super TransformedInstance> consumer) {
		...
		for (MyInstanceTree child : children) {
	...
	public void copyTransform(MyInstanceTree tree) {
		x = tree.x;
		y = tree.y;
		z = tree.z;
		xRot = tree.xRot;
